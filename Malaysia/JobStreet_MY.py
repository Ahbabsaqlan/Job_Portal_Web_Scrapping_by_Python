...
def create_session_with_retries():
    ...
    retry_strategy = Retry(total=5, backoff_factor=2, status_forcelist=[429, 500, 502, 503, 504], allowed_methods=["HEAD", "GET", "POST"], respect_retry_after_header=True)
...
def final_robust_parser(html_content):
    ...
    try:
        ...
        # FIX: Return a dictionary of lists, not joined strings
        return dict(results)
...
def fetch_job_details(session, job_id):
    ...
    try:
        resp = session.post(DETAILS_API_URL, headers=headers, json=payload, timeout=30)
...
def collect_new_jobstreet_ids_until_target_met(session, existing_ids, target_count):
    headers = {"User-Agent": "Mozilla/5.0", "Accept": "application/json"}
    newly_found_ids, issues = set(), []
    try:
        params = {"siteKey": SITE_KEY, "page": 1, "pageSize": PAGE_SIZE, "classification": CLASSIFICATIONS}
        resp = session.get(ID_SEARCH_URL, headers=headers, params=params, timeout=20)
        ...
        issues.append(f"Critical ID Collection network failure: {e}")
        logging.critical(f"❌ ID Collection failed critically due to network: {e}", exc_info=False)
        return [], issues
    except Exception as e:
...
def main():
    ...
    new_job_ids, issues = collect_new_jobstreet_ids_until_target_met(session, existing_ids, PROCESS_LIMIT)
    ...
    for job_id in tqdm(jobs_to_scrape, desc=f"Scraping {DISPLAY_NAME} Details"):
        job_details = fetch_job_details(session, job_id)
