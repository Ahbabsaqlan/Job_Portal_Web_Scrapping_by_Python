...
# ================= CONFIGURATION: PHILIPPINES (PH) ================= #
BASE_DIR = "/Users/jihan/JobWebScrapper"
LOG_DIR = os.path.join(BASE_DIR, "logs")
SUMMARY_FILE = os.path.join(BASE_DIR, "run_summary.json")

# --- Country-Specific Settings ---
HOST = "https://ph.jobstreet.com"
SITE_KEY = "PH-Main"
LOCALE = "en-PH"
TIMEZONE = "Asia/Manila"
MASTER_FILE = os.path.join(BASE_DIR, "Dataset/jobstreet_ph_master_data.xlsx")
LOG_FILE_PREFIX = "jobstreet_ph_run"
DISPLAY_NAME = "JobStreet (PH)"
API_HEADERS = {"seek-request-brand": "jobstreet", "seek-request-country": "PH", "X-Seek-Site": "chalice"}
CLASSIFICATIONS = "1200,1203,1204,6008,6251,6043,1206,1209,6076,1210,1211,6092,1212,1214,6123,1216,6163,1220,1223,6205,6246,6261,6263,6281,6304,1225,6058,7019,6362"
...
def fetch_job_details(job_id):
    referer_url = f"{HOST}/job/{job_id}"
    headers = { "User-Agent": "Mozilla/5.0", "Content-Type": "application/json", "Origin": HOST, "Referer": referer_url }
    headers.update(API_HEADERS)
    
    # ========================== THE FINAL FIX ==========================
    # This payload now perfectly matches the structure of a successful browser request
    # for the PH site. It uses the 'jobDetails' wrapper but with simplified fields inside.
    payload = {
    ...
    try:
        ...
        # The data path is now confirmed to be data -> jobDetails -> job
...
def collect_new_jobstreet_ids_until_target_met(existing_ids, target_count):
    headers = { "User-Agent": "Mozilla/5.0", "Accept": "application/json", "Referer": f"{HOST}/jobs" }
    ...
    try:
        ...
        for page in tqdm(range(1, total_pages + 1), desc=f"Collecting new {DISPLAY_NAME} IDs"):
            if page > 1:
                params['page'] = page
                headers["Referer"] = f"{HOST}/jobs?page={page}"
                try:
                    resp = requests.get(ID_SEARCH_URL, headers=headers, params=params, timeout=20)
                    resp.raise_for_status(); data = resp.json()
                except Exception as e:
                    issues.append(f"Failed to fetch ID page {page}: {e}")
                    continue
